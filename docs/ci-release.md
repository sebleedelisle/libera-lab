...
example `v1.2.1-beta`, `v1.2.1-rc`, or `v1.2.1-private`. The build number uses
the GitHub Actions run number unless the tag already includes a full identity
such as `v1.2.1-beta.123`.
...
- The licence server currently imports `libera-lab-macos.dmg` and
  `libera-lab-windows.zip` for app code `LAB01`; Linux is still built and
