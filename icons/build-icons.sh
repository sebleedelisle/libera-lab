...
SOURCE_PNG="${ROOT_DIR}/libera-lab.png"
ICONSET_BASE="$(mktemp -d "${TMPDIR:-/tmp}/libera-lab.XXXXXX")"
...
iconutil -c icns "${ICONSET_DIR}" -o "${ROOT_DIR}/libera-lab.icns"
magick "${SOURCE_PNG}" -define icon:auto-resize=256,128,96,64,48,32,16 "${ROOT_DIR}/libera-lab.ico"
